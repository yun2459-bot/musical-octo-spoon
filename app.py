...
with tab4:
    if not HW.available():
        ...
        if obs.empty:
            ...
            with st.expander("⚖️ 고용노동부 공식 기준과 비교"):
                st.markdown(
                    "**세방 SAFETY TF(사내 기준)**: 주의 33℃ · 경고 35℃ · 위험 38℃\n\n"
                    "**고용노동부 공식(2025.7.17 안전보건규칙 개정 · 산업안전보건법 제39조1항7호, "
                    "2025.6 시행)**: 관심 25℃(사전점검·교육 권장) · 주의 31℃(보건조치 의무 발생 — "
                    "냉방/통풍장치, 작업시간 조정, 휴식시간 부여 중 1개 이상) · 경계 33℃(2시간 이내 "
                    "20분 이상 휴식 **의무**) · 심각 38℃(작업중지 권고 강화)"
                )
                st.warning(
                    "⚠️ **31~32℃ 구간은 정부 기준으로는 이미 '주의' 단계(보건조치 법적 의무 발생)인데, "
                    "사내 기준으로는 이 대시보드에 '정상'(초록색)으로 표시됩니다.** 중대재해처벌법·"
                    "산업안전보건법 대응 근거자료로 쓰실 계획이라면, 사내 기준을 정부 4단계에 맞추는 "
                    "것도 검토해보시면 좋을 것 같습니다(임계값을 실제로 바꾸는 건 알림 발송 시점이 "
                    "바뀌는 운영상 결정이라 제가 임의로 바꾸지 않았습니다).",
                    icon="⚠️",
                )
                st.caption("출처: 고용노동부 정책자료(2025.7), 안전보건규칙 제560·562조 — 법령 원문은 "
                           "moel.go.kr에서 직접 확인하시길 권합니다.")
