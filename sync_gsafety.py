...
def filter_branch_records(df: pd.DataFrame) -> pd.DataFrame:
    """'지사 자체 점검'만 남긴다: 점검유형=지사 주관 + 조치부서가 'OO지사' 형태 + 코칭부서==조치부서.
    코칭부서==조치부서 요건은 그룹합동안전점검TF/CSO EHS팀처럼 본사 조직이 여러 지사를 순회
    코칭한 건(코칭부서≠조치부서)을 제외하기 위함 — 그런 건은 조치부서만 보면 그 지사 안전관리자가
    스스로 점검한 것처럼 보이지만 실제로는 본사 합동점검이라, 편향분석(다양성지수 등)에 섞이면
    지사 고유의 점검 습관 신호가 왜곡된다 (2026-08-24 기준, clean_data.py와 동일 기준 적용)."""
    mask = ((df["점검유형"] == "지사 주관")
            & (df["코칭부서"] == df["조치부서"])
            & df["조치부서"].astype(str).str.match(BRANCH_DEPT_RE))
    return df[mask].copy()
